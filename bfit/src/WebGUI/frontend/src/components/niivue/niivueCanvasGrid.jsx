...
export default function NiivueCanvasGrid({
        ...
        const handleNvLocationChange = (data) => {
          ...
          const sliceNumber =
            index === NVSLICE_TYPES.AXIAL
              ? clampSliceNumber(Math.round(data.vox?.[2] || 0) + 1, currentViewMax)
              : getVoxelSliceNumber(data.vox, index, currentViewMax);
  ...
  // ─── Track viewer slice position from the NiiVue instances ───────────────
  useEffect(() => {
  ...
  }, [isVolumeLoaded, nvInstances, volMax, setCurrentSlice]);
