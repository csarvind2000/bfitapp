...
const cloneDrawBitmap = (nv) => {
  if (!nv?.drawBitmap) return null;
  return new Uint8Array(nv.drawBitmap);
};

const drawBitmapsEqual = (a, b) => {
...
const restoreDrawBitmap = (nv, snapshot) => {
  if (!nv || !snapshot) return false;
  nv.drawBitmap = new Uint8Array(snapshot);
  refreshDrawing(nv);
  return true;
};

const setNvCrosshairVisibility = (nv, enabled) => {
  if (!nv) return;
  if (nv.opts) nv.opts.show3Dcrosshair = enabled;
  nv.setCrosshairWidth?.(enabled ? 0.5 : 0);
  nv.drawScene?.();
};
...
export default function NiivueToolbar({
  ...
  const { selectedCanvasId, nvInstances, setAddedSegmentation, activeMaskType, addScreenshotToGallery } =
    useNiivueStore(
      useShallow((state) => ({
  ...
  const lastDrawBitmapRef = useRef(null);
  ...
  const applyToNvInstances = (callback) => {
    Object.values(nvInstances || {}).forEach((targetNv) => {
      if (targetNv) callback(targetNv);
    });
  };

  useEffect(() => {
    Object.values(nvInstances || {}).forEach((targetNv) => {
      setNvCrosshairVisibility(targetNv, crosshairEnabled);
    });
  }, [nvInstances, crosshairEnabled]);

  useEffect(() => {
    if (!nv) {
      lastDrawBitmapRef.current = null;
      return undefined;
    }

    lastDrawBitmapRef.current = cloneDrawBitmap(nv);
    const previousOnMouseUp = nv.onMouseUp;

    const handleMouseUp = (...args) => {
      const before = lastDrawBitmapRef.current;
      const after = cloneDrawBitmap(nv);
      if (before && after && !drawBitmapsEqual(before, after)) {
        setRedoStack([]);
      }
      lastDrawBitmapRef.current = after;
      if (typeof previousOnMouseUp === "function") {
        previousOnMouseUp.apply(nv, args);
      }
    };

    nv.onMouseUp = handleMouseUp;
    return () => {
      if (nv.onMouseUp === handleMouseUp) {
        nv.onMouseUp = previousOnMouseUp;
      }
    };
  }, [nv]);

  useEffect(() => {
    if (interactionMode !== "pan") return undefined;

    const cleanups = Object.values(nvInstances || {}).flatMap((targetNv) => {
      const canvas = targetNv?.gl?.canvas;
      if (!canvas) return [];
      ...
      const handlePanMouseDown = (e) => {
        ...
        targetNv.drawPenLocation = [NaN, NaN, NaN];
        targetNv.drawPenAxCorSag = -1;
        targetNv.uiData.mousedown = true;
        targetNv.setDragStart?.(0, 0);
        targetNv.setDragEnd?.(0, 0);

        const pos = targetNv.getNoPaddingNoBorderCanvasRelativeMousePosition?.(e, canvas);
        if (!pos) return;

        const x = pos.x * targetNv.uiData.dpr;
        const y = pos.y * targetNv.uiData.dpr;
        targetNv.uiData.clickedTile = targetNv.tileIndex?.(x, y);
        targetNv.uiData.mouseButtonCenterDown = true;
        targetNv.mouseCenterButtonHandler?.(e);
      ...
      canvas.addEventListener("mousedown", handlePanMouseDown, true);
      canvas.addEventListener("mouseup", handlePanMouseUp, true);
      canvas.addEventListener("mouseleave", handlePanMouseUp, true);

      return [
        () => canvas.removeEventListener("mousedown", handlePanMouseDown, true),
        () => canvas.removeEventListener("mouseup", handlePanMouseUp, true),
        () => canvas.removeEventListener("mouseleave", handlePanMouseUp, true),
      ];
    });

    return () => cleanups.forEach((cleanup) => cleanup());
  }, [interactionMode, nvInstances]);
  ...
  const handleShowAll = (e) => {
    ...
    e.preventDefault();
    setLabelMenuAnchor(null);
    onShowFullMask?.(activeVariant ?? "5class");
  };
  ...
  const activateDraw = () => {
    if (!nv) return;
    applyToNvInstances((targetNv) => {
      setNvCrosshairVisibility(targetNv, crosshairEnabled);
      if (targetNv.opts) targetNv.opts.dragMode = DRAG_MODE.none;
      if (targetNv.gl?.canvas) targetNv.gl.canvas.style.cursor = "default";
    });
  ...
  const deactivateTools = () => {
    ...
    setNvCrosshairVisibility(nv, crosshairEnabled);
    nv.updateGLVolume?.();
    nv.drawScene?.();
    setDrawMode(null);
  ...
  const handleCrosshairClick = () => {
    const shouldEnableCrosshair = !crosshairEnabled;
    applyToNvInstances((targetNv) => {
      targetNv.setDrawingEnabled?.(false);
      targetNv.setPenValue?.(-1);
      setNvCrosshairVisibility(targetNv, shouldEnableCrosshair);
      if (targetNv.opts) targetNv.opts.dragMode = DRAG_MODE.none;
      if (targetNv.gl?.canvas) targetNv.gl.canvas.style.cursor = "default";
    ...
    setDrawMode(null);
    setInteractionMode(shouldEnableCrosshair ? "crosshair" : "idle");
    setCrosshairEnabled(shouldEnableCrosshair);
  };

  const handlePanClick = () => {
    const shouldEnablePan = interactionMode !== "pan";
    applyToNvInstances((targetNv) => {
      targetNv.setDrawingEnabled?.(false);
      targetNv.setPenValue?.(-1);
      setNvCrosshairVisibility(targetNv, crosshairEnabled);
      if (targetNv.opts) targetNv.opts.dragMode = shouldEnablePan ? DRAG_MODE.pan : DRAG_MODE.none;
      if (targetNv.gl?.canvas) targetNv.gl.canvas.style.cursor = shouldEnablePan ? "grab" : "default";
      targetNv.updateGLVolume?.();
      targetNv.drawScene?.();
    });
    setDrawMode(null);
    setInteractionMode(shouldEnablePan ? "pan" : crosshairEnabled ? "crosshair" : "idle");
  };
  ...
  const handleUndo = () => {
    if (!nv) return;
    const before = cloneDrawBitmap(nv);
    if (!before) return;

    const undoIndexBefore =
      typeof nv.currentDrawUndoBitmap === "number" ? nv.currentDrawUndoBitmap : null;
    const maxAttempts = Math.max(1, (nv.drawUndoBitmaps?.length || 0) + 1);
    let changed = false;

    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      nv.drawUndo?.();
      const after = cloneDrawBitmap(nv);
      if (after && !drawBitmapsEqual(before, after)) {
        changed = true;
        break;
      }
    }

    if (!changed && undoIndexBefore !== null) {
      nv.currentDrawUndoBitmap = undoIndexBefore;
      restoreDrawBitmap(nv, before);
      return;
    }

    lastDrawBitmapRef.current = cloneDrawBitmap(nv);
    if (changed) {
      setRedoStack((prev) => [...prev, before].slice(-20));
    }
  };
  ...
  const handleRedo = () => {
    ...
    if (!restoreDrawBitmap(nv, snapshot)) return;
  ...
  const addCanvasToGallery = (canvas, viewLabel) => {
  ...
  const capturePageSelectionToCanvas = async (cropRect) => {
    if (!navigator.mediaDevices?.getDisplayMedia) {
      throw new Error("Page capture is not supported by this browser");
    }
  ...
  const handleScreenshotViewer = async () => {
    ...
    try {
      addCanvasToGallery(await renderTargetsToCanvas(targets), "Whole viewer");
      showAlert("Viewer screenshot saved to gallery", "success");
    } catch (err) {
  ...
  const finishManualSelection = async (start, end) => {
    ...
    try {
      ...
      let selectedCanvas;
      try {
        selectedCanvas = await capturePageSelectionToCanvas(cropRect);
      } catch (pageCaptureError) {
        console.warn("Page screenshot fallback to viewer capture:", pageCaptureError);
        selectedCanvas = await renderTargetsToCanvas(selectionTargetsRef.current, cropRect);
      }
      addCanvasToGallery(selectedCanvas, "Selected box");
      showAlert("Selection screenshot saved to gallery", "success");
    } catch (err) {
  ...
  const renderLabelRow = ([key, value]) => {
    ...
    const labelName = String(value ?? "");
    const isBackground = labelName.toLowerCase() === "background";
    ...
    return (
      ...
      >
        ...
        <Typography sx={{ fontSize: 14, color: "#e2e2e2", flex: 1, lineHeight: 1 }}>
          {getDisplayName(labelName, activeMaskType)}
        </Typography>
  ...
  const isCrosshair = crosshairEnabled;
    ...
    <Stack direction="row" alignItems="center" sx={toolbarSx}>
      ...
      <Box sx={toolGroupSx}>
        <Tooltip title="Crosshair">
          <Box component="button" onClick={handleCrosshairClick} sx={toggleBtnSx(isCrosshair)}>
            <GpsFixedIcon />
          </Box>
        </Tooltip>
        <Tooltip title="Pan">
          <Box component="button" onClick={handlePanClick} sx={toggleBtnSx(isPan)}>
            <PanToolAltIcon />
          </Box>
        </Tooltip>
