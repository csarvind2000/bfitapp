 ...
 */
function buildMultiLabelLut(visibleIndices, opacity = 180) {
  ...
  for (let i = 1; i < srcLen; i++) {
    ...
    lut[offset + 3] = isVisible ? opacity : 0;
  }
  return lut;
}
...
export default function useLabelMaskManager(nvInstancesInput, maskOpacity) {
  ...
  const getAlpha = useCallback(() => {
    const op = maskOpacityRef.current;
    if (typeof op === "number" && op <= 1) return Math.round(op * 255);
    if (typeof op === "number") return Math.min(255, Math.round(op));
    return 180;
  }, []);
  ...
  const loadMask = useCallback(
    async (activeMaskType, label, _fileInfo) => {
      ...
      applyDrawLutToAll(allNVs, buildMultiLabelLut(activeLabelIndices.current, getAlpha()));
    },
    [getAllNVs, getAlpha]
  );
  ...
  const removeMask = useCallback(
    (activeMaskType, label) => {
      ...
      if (activeMaskType === null && label === null) {
        ...
        applyDrawLutToAll(allNVs, buildMultiLabelLut(new Set(), getAlpha()));
        return;
      }
  ...
  const updateOpacity = useCallback(() => {
    applyDrawLutToAll(
      getAllNVs(),
      buildMultiLabelLut(activeLabelIndices.current, getAlpha())
    );
  }, [getAllNVs, getAlpha]);

  return { loadMask, removeMask, updateOpacity };
}