    ...
    <Stack direction="row" spacing={1} alignItems="center">
      <Box sx={{ textAlign: "left" }}>
        ...
        <Typography title={seriesId} variant="caption"
          sx={{ opacity: 0.8, fontSize: "0.6rem", lineHeight: 1.15, textWrap: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {seriesId}
        </Typography>
      </Box>
    </Stack>
    ...
    async function getNVImage() {
      ...
      if (response.artifacts && response.artifacts.length > 0) {
        const defaultArtifact =
          response.artifacts.find((artifact) => artifact.artifact_type === AnalysisResultTypes.ORIGINAL_MR) ||
          response.artifacts.find((artifact) => artifact.artifact_type === AnalysisResultTypes.ORIGINAL_MR_INPHASE) ||
          response.artifacts[0];
        const nv = await NVImage.loadFromUrl({ url: defaultArtifact.artifact_url });
        setNvImage(nv);
      } else {
      ...
      >
        <Box
          sx={{
            ...
            py: 0.75,
            bgcolor: "#1f2631",
            borderBottom: galleryCollapsed ? "none" : "1px solid rgba(148, 163, 184, 0.22)",
          }}
        >
          <Typography variant="body2" sx={{ fontWeight: 600, color: "#f8fafc" }}>
            Gallery
          </Typography>
          <Typography variant="body2" sx={{ ml: "auto", color: "#f8fafc", fontWeight: 600 }}>
            {screenshotGallery.length} image(s)
          </Typography>
          <IconButton
            size="small"
            aria-label={galleryCollapsed ? "Expand gallery" : "Collapse gallery"}
            onClick={() => setGalleryCollapsed((collapsed) => !collapsed)}
            sx={{
              ml: 0.5,
              width: 26,
              height: 26,
        ...
        <Collapse in={!galleryCollapsed} timeout="auto">
          {screenshotGallery.length === 0 ? (
            ...
            >
              ...
              >
                <ImageIcon sx={{ fontSize: 16, color: "#e2e8f0" }} />
              </Box>
              <Typography variant="body2" sx={{ fontWeight: 600, lineHeight: 1.25 }}>
                No images captured yet
              </Typography>
              <Typography variant="body2" sx={{ maxWidth: 250, lineHeight: 1.35 }}>
                Use the capture button in the panel of the DICOM viewer to add images here
              </Typography>
            </Box>
          ) : (
...
function LeftDrawerForSegmentation({
  ...
  return (
    <Drawer
      sx={{
        ...
        "& .MuiDrawer-paper": {
          ...
          flexDirection: "column",
          height: "100vh",
          bgcolor: viewerChrome.background,
          color: viewerChrome.text,
          borderRight: `1px solid ${viewerChrome.border}`,
          "& .MuiDivider-root": { borderColor: viewerChrome.border },
        },
        overflow: "hidden",
      }}
      PaperProps={{
    ...
    >
      <DrawerHeader />
      {leftDrawerCollapsed ? (
        <IconButton onClick={handleLeftDrawerToggle} sx={{ borderRadius: 1.5, color: viewerChrome.muted }}><ChevronRight /></IconButton>
      ) : (
        <Box sx={{ display: "flex", alignItems: "center", flexDirection: "row-reverse" }}>
          <IconButton onClick={handleLeftDrawerToggle} sx={{ borderRadius: 1.5, marginLeft: "auto", color: viewerChrome.muted }}><ChevronLeft /></IconButton>
          <Typography variant="body2" sx={{ marginLeft: "auto", color: viewerChrome.muted }} fontWeight={500}>
            Segmentations
          </Typography>
        </Box>
      )}
      ...
      <Collapse in={!leftDrawerCollapsed} timeout="auto" sx={{ overflowX: "hidden" }}>
        ...
        <AnalysisResultToolGrid
          ...
          onBoundedMaskSaved={onBoundedMaskSaved}
        />
        <Divider sx={{ mt: 1 }} />
  ...
  return (
    <Drawer
      sx={{
        ...
        "& .MuiDrawer-paper": {
          ...
          bgcolor: viewerChrome.background,
          color: viewerChrome.text,
          borderLeft: `1px solid ${viewerChrome.border}`,
          "& .MuiDivider-root": { borderColor: viewerChrome.border },
        },
      }}
      PaperProps={{
    ...
    >
      ...
      {rightDrawerCollapsed ? (
        <IconButton onClick={handleRightDrawerToggle} sx={{ borderRadius: 1.5, color: viewerChrome.muted }}><ChevronLeft /></IconButton>
      ) : (
        <Box display="flex">
          <IconButton onClick={handleRightDrawerToggle} sx={{ borderRadius: 1.5, justifyContent: "flex-start", color: viewerChrome.muted }}><ChevronRight /></IconButton>
          <Typography variant="body2" margin="auto" fontWeight={500} sx={{ color: viewerChrome.muted }}>Analysis</Typography>
        </Box>
      )}
      <Divider />
      <Collapse
        in={!rightDrawerCollapsed}
        timeout="auto"
        sx={{
          flex: 1,
          minHeight: 0,
...
export default function AnalysisResultModal({
  ...
  const [selectedVariant, setSelectedVariant] = useState("47class");
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      setScreenshotGalleryKey: state.setScreenshotGalleryKey,
    }))
  );
  ...
  useEffect(() => {
    ...
    setScreenshotGalleryKey(analysisId);
    return () => setScreenshotGalleryKey(null);
  }, [analysisId, setScreenshotGalleryKey]);
  ...
  const { loadMask, removeMask, updateOpacity, hideAllMasks } = useLabelMaskManager(nvInstancesRef, maskOpacity);

  useEffect(() => {
    updateOpacity();
  }, [maskOpacity, updateOpacity]);
  ...
  const handleClearLabelFilters = useCallback(() => {
  ...
  const captureReportCrosshairState = () =>
    getReportViewInstances().map(({ index, nv }) => ({
      index,
      crosshairWidth: nv?.opts?.crosshairWidth,
      show3Dcrosshair: nv?.opts?.show3Dcrosshair,
    }));

  const setReportCrosshairVisibility = async (visible) => {
    getReportViewInstances().forEach(({ nv }) => {
      if (!nv) return;
      try {
        if (nv.opts) nv.opts.show3Dcrosshair = visible;
        nv.setCrosshairWidth?.(visible ? 0.5 : 0);
        safeReportRedraw(nv, "Report crosshair visibility");
      } catch (error) {
        console.warn("Report crosshair visibility skipped:", error);
      }
    });
    await waitForViewerRender();
  };

  const restoreReportCrosshairState = async (states) => {
    for (const { index, crosshairWidth, show3Dcrosshair } of states || []) {
      const nv = nvInstancesRef.current?.[index];
      if (!nv) continue;
      try {
        if (typeof show3Dcrosshair === "boolean" && nv.opts) {
          nv.opts.show3Dcrosshair = show3Dcrosshair;
        }
        if (typeof crosshairWidth === "number") {
          nv.setCrosshairWidth?.(crosshairWidth);
        }
        safeReportRedraw(nv, "Restored report crosshair");
      } catch (error) {
        console.warn("Report crosshair restore skipped:", error);
      }
    }
    await waitForViewerRender();
  };
  ...
  const getReportSliceText = (view, nv) => {
    if (view?.label !== "Axial") return null;

    const dims = nv?.volumes?.[0]?.dimsRAS;
    const crosshair = nv?.scene?.crosshairPos;
    if (!dims || !crosshair) return null;

    const totalSlices = Number(dims[3]);
    if (!Number.isFinite(totalSlices) || totalSlices <= 0) return null;

    const slice = Math.min(
      totalSlices,
      Math.max(1, Math.ceil(Number(crosshair[2] || 0) * totalSlices))
    );
    return `I: ${slice} (${slice}/${totalSlices})`;
  };

  const captureReportCanvasDataUrl = (view, nv, canvas) => {
    const sliceText = getReportSliceText(view, nv);
    if (!sliceText) {
    ...
    stampedCanvas.height = canvas.height;
    ...
    const paddingY = Math.max(6, Math.round(fontSize * 0.35));
    const x = Math.max(12, Math.round(canvas.width * 0.014));
    const y = Math.max(10, Math.round(canvas.height * 0.012));

    context.font = `700 ${fontSize}px Arial, sans-serif`;
    context.textBaseline = "top";
    const textMetrics = context.measureText(sliceText);
    context.fillStyle = "rgba(0, 0, 0, 0.62)";
    context.fillRect(
      x - paddingX * 0.5,
      y - paddingY * 0.5,
      textMetrics.width + paddingX,
      fontSize + paddingY
    );
    context.fillStyle = "#00e5ff";
    context.fillText(sliceText, x, y);

    return stampedCanvas.toDataURL("image/png");
  };
  ...
  const captureReportViews = async () => {
    ...
    for (const view of reportViews) {
      ...
      captures.push({
        label: view.label,
        dataUrl: captureReportCanvasDataUrl(view, nv, canvas),
      });
    }
    return captures;
  };
  ...
  const handleGenerateBodyAnalysisReport = async () => {
    ...
    const segmentations = (analysisResult.segmentations || []).filter(
      (segmentation) => !String(segmentation?.mask_type || "").toUpperCase().includes("4CLASS")
    );
    ...
    let originalReportCrosshairState = [];
    ...
    try {
      ...
      originalReportCrosshairState = captureReportCrosshairState();
      await setReportCrosshairVisibility(false);
      ...
      for (const segmentation of segmentations) {
        ...
        sections.push({
          maskName: getMaskDisplayName(segmentation.mask_type),
          inputImages,
          overlayImages,
          ...
          subMuscleRows: buildSubMuscleRows(analysisResult, segmentation.mask_type).map((row) => ({
            ...row,
            left: row.left
              ? {
                  ...row.left,
                  volume: formatVolume(row.left.volume),
                  percent: formatPercent(row.left.percent),
                }
              : null,
            right: row.right
              ? {
                  ...row.right,
                  volume: formatVolume(row.right.volume),
                  percent: formatPercent(row.right.percent),
                }
              : null,
          })),
        });
      ...
      let summary = "";
      try {
        const summaryResponse = await analysisService.loadSummary(patient.id || analysisId);
        summary = summaryResponse?.summary || "";
      } catch (error) {
        console.warn("Report summary could not be loaded:", error);
      }
      ...
      const reportName =
        ...
        analysisId;
      showAlert(
        `Report saved for ${reportName}. Open View all Reports to view it.`,
        "success"
      );
    } catch (error) {
      ...
      try {
        await restoreReportCrosshairState(originalReportCrosshairState);
      } catch (error) {
        console.warn("Report crosshair cleanup skipped:", error);
      }
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <Box sx={{ minWidth: 0, display: "flex", alignItems: "center" }}>
              <IconButton edge="start" color="inherit" onClick={onClose} aria-label="close"><ArrowBackIcon /></IconButton>
            ...
            <Box
              sx={{
                ...
                justifyContent: "center",
                maxWidth: "min(1000px, calc(100vw - 520px))",
                minWidth: 0,
              }}
            >
              {analysisResult && (
            ...
            <Box sx={{ display: { xs: "none", md: "flex" }, justifyContent: "flex-end", minWidth: 0 }}>
              ...
              >
                <Button autoFocus color="secondary" startIcon={<CommentIcon />} onClick={() => setIsOpenComment(true)}>
                  Comment
                </Button>
              </Badge>
            </Box>
        ...
        <LeftDrawerForSegmentation
          ...
          setMaskOpacity={setMaskOpacity}
          nvInstancesRef={nvInstancesRef}
          onHideLabels={handleClearLabelFilters}
          showHoverLabel={showHoverLabel}
          onToggleHoverLabel={() => setShowHoverLabel((v) => !v)}
          onBoundedMaskSaved={refreshAnalysisResult}
        />
