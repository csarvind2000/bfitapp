...
const MaskBoundModal = ({
  ...
  useEffect(() => {
    if (mounted && sourceVolume && !isNvAttached.current) {
      const slices = sourceVolume.dims[3];
      setBounds([1, slices]);
      setUpperBound(slices);
      nvInstance.addVolume(sourceVolume);
  ...
  }, [sourceVolume, mounted, nvInstance]);

  const volumeDims = sourceVolume?.dims ?? null;

  useEffect(() => {
    let isExpired = false;
    if (nvInstance && isNvAttached.current && sourceMask && volumeDims) {
      nvInstance.loadDrawingFromUrl(sourceMask).then(() => {
        if (isExpired) return;
  ...
  }, [debouncedBounds, sourceMask, volumeDims, nvInstance]);
  ...
  const boundMask = async () => {
    try {
      const gzippedData = pako.gzip(sourceMask);
      ...
      const boundedMask = await analysisService.getBounded(formData);
      await applyBoundedMask(boundedMask.file_data);
    } catch (error) {
      showAlert(`Failed to bound mask: ${JSON.stringify(error.response?.data)}`, "error");
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button
          disabled={!mounted || !sourceMask}
          onClick={async () => {
            if (!sourceMask) { showAlert("Mask not selected", "warning"); return; }
            setLoading(true);
            await boundMask();
            setLoading(false);
            closeDialog();
