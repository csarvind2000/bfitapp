...
export default function UploadModal({ open, onClose, getStudy }) {
  ...
  const generateUploadSummaryMessage = (response, analysisMessage = "") => {
    const studyIds = Object.keys(response);
    const numStudies = studyIds.length;
    let html;
    if (numStudies === 0) {
      return analysisMessage
        ? `No studies were processed. ${analysisMessage}`
        : "No studies were processed";
    } else {
      html =
        numStudies > 1
    ...
    html += "</ul>";
    if (analysisMessage) {
      html += `<p><strong>${analysisMessage}</strong></p>`;
    }
  ...
  const getUploadedSeriesIds = (response) =>
    Object.values(response || {}).flatMap((series) => Object.keys(series || {}));
  ...
  const handleUpload = React.useCallback(async () => {
    ...
    try {
      ...
      if (response) {
        const seriesIds = getUploadedSeriesIds(response);
        const failedSeriesIds = [];

        for (const seriesId of seriesIds) {
          try {
            await analysisService.create(seriesId);
          } catch (error) {
            failedSeriesIds.push(seriesId);
            console.error(`Failed to start analysis for ${seriesId}`, error);
          }
        }
        ...
        const analysisMessage =
          seriesIds.length === 0
            ? ""
            : failedSeriesIds.length === 0
              ? `Analysis started for ${seriesIds.length} uploaded series`
              : `Uploaded successfully, but analysis failed to start for ${failedSeriesIds.length} series`;
        const message = generateUploadSummaryMessage(response, analysisMessage);
        const severity =
          failedSeriesIds.length > 0
            ? "warning"
            : seriesIds.length > 0
