...
import { DateFormatter, getAnalysisTypeLabel } from "../constants";
  ...
  const columns = useMemo(
    () => [
      { field: "id", headerName: "ID", width: 160, sortable: false },
      {
        ...
        width: 170,
        editable: false,
        valueGetter: (value, row) => row.patient_name || row.patient_id || "-",
      },
      {
        field: "queue",
        headerName: "Type",
        width: 130,
        editable: false,
        valueGetter: (value, row) => getAnalysisTypeLabel(row),
      },
      {
        field: "series",
