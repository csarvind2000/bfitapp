...
export default function AnalysisResultTable({ analysisResult }) {
  ...
  useEffect(() => {
    ...
    if (!selectedPrediction && analysisResult?.predictions?.length) {
      ...
      if (key === "4class") selectedPrediction = prediction["4class"]?.[0];
      else if (key === "5class") selectedPrediction = prediction["5class"]?.[0];
    ...
    Object.keys(selectedPrediction).forEach((csvKey) => {
      ...
      if (isSimpleVariant) {
        if (key === "4class" && base.includes("organ")) return;
        ...
        <>
          ...
          <Collapse in={showSubMuscles} timeout="auto" unmountOnExit>
            ...
            <TableContainer component={Paper} elevation={0} sx={{
              ...
              <TableBody>
                {Object.entries(muscleTable).length === 0 ? (
                  <TableRow>
