  ...
  useEffect(() => {
    ...
    if (!selectedPrediction && analysisResult?.predictions?.length) {
      ...
      if (key === "5class") selectedPrediction = prediction["5class"]?.[0];
        ...
        selectedPrediction = prediction["47class"]?.[0] ?? prediction["48class"]?.[0];
      else if (key === "abd_mr")
        selectedPrediction =
          prediction["abd_mr"]?.[0] ??
    ...
    const main = {};
    const muscles = {};
    const isSimpleVariant = key !== "47class";
    ...
    Object.keys(selectedPrediction).forEach((csvKey) => {
      ...
      // Detailed thigh: split bilateral aggregates vs per-side muscles
