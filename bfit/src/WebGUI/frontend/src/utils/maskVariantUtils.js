...
export const VARIANT_CONFIG = [
  ...
  {
    key: "47class",
    title: "MR Thigh Fat & Sub-Muscle Analysis",
    tokens: ["47CLASS", "47_CLASS", "48CLASS", "48_CLASS"],
    labels: Labels,
    displayNames: null,
    colors: null,
    mainLabelKeys: ["bone", "imat", "sat", "imf", "organ", "groin_region", "background"],
    showMuscleSubTable: true,
    groups: {
      Basic: ["background", "bone", "sat", "imat", "imf", "organ", "groin_region"],
      Bones: ["femur", "ilium"],
  ...
  {
    key: "5class",
    title: "MR Thigh Fat & Muscle Analysis",
...
/** Canonical key: "abd_mr" | "47class" | "5class" | null */
export function getVariantKey(activeMaskType) {
  return getVariantConfig(activeMaskType)?.key ?? null;
}

/** User-facing mask name. Keeps backend mask_type values unchanged. */
export function getMaskDisplayName(activeMaskType) {
  if (Array.isArray(activeMaskType)) {
    return activeMaskType.map((maskType) => getMaskDisplayName(maskType)).join(" + ");
  }
  const fallback = String(activeMaskType || "Mask")
    .replace(/_/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return getVariantConfig(activeMaskType)?.title ?? fallback;
}

/** Label map (index → name) for the active mask type. Falls back to detailed thigh. */
...
export function isDetailedThighVariant(activeMaskType) {
  return getVariantKey(activeMaskType) === "47class";
}

/** @deprecated use isDetailedThighVariant */
export function is48ClassVariant(activeMaskType) {
  return isDetailedThighVariant(activeMaskType);
}
...
export function getDisplayName(labelName, activeMaskType) {
  ...
  let base = String(labelName ?? ""), side = "";
  if (base.endsWith("_left"))  { side = " (L)"; base = base.replace(/_left$/, ""); }
  if (base.endsWith("_right")) { side = " (R)"; base = base.replace(/_right$/, ""); }
  return base.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) + side;
}
 ...
 * Anything not in mainLabelKeys collapses to "muscle" in simple views.
 */
