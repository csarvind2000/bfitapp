...
const getSourceScan = async (analysisId, channel) => {
  const response = await axiosClient.post(
    `/${analysisId}/source-scan/`,
    { channel },
    { useAuthorization: true }
  );
  return response.data;
...
const deleteComment = async (analysisId, index = null) => {
  const response = await axiosClient.delete("/delete_comment/", {
    params: { analysis_id: analysisId, ...(index !== null ? { index } : {}) },
    useAuthorization: true,
  });
