...
import { buildToolpadSession, SessionContext } from "./hooks/session";
import authService from "./services/auth";

const TOKEN_EXPIRY_MS = 24 * 60 * 60 * 1000;

function getTokenExpiry(expiresAt: unknown) {
...
export default function App() {
    ...
    (async () => {
      ...
      try {
        ...
        if (sess) {
          ...
          const expiresAt = getTokenExpiry(sessObj.expiresAt);

          if (!sessObj.token || expiresAt <= Date.now()) {
            throw new Error("Auth token expired");
          }

          const verifiedSession = await authService.verify(sessObj.token);
          const authSession = { ...sessObj, ...verifiedSession, expiresAt };
          window.localStorage.setItem("session", JSON.stringify(authSession));
          setSession(buildToolpadSession(authSession));
          navigate(url, {
            replace: true,
            viewTransition: true,
          });
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("bfit:auth-invalid", handleInvalidAuth);
    return () => window.removeEventListener("bfit:auth-invalid", handleInvalidAuth);
  }, [navigate]);
