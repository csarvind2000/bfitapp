...
test("body-analysis report generation is wired through segmentation data", () => {
  const source = readProjectFile("src/components/analysisResultModal.jsx");
  ...
  assert.match(source, /!analysisResult\?\.segmentations\?\.length/);
  assert.match(source, /handleGenerateBodyAnalysisReport\(\)\.finally/);
  assert.doesNotMatch(source, /disabled=\{reportGenerating \|\| !.*analysisResult\?\.analysis\.queue/);
});
