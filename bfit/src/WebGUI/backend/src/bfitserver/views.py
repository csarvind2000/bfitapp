...
def _report_add_sub_muscle_table(pdf, rows):
    ...
    _report_ensure_space(pdf, 28)
    pdf.set_font("Helvetica", "B", 8)
    pdf.set_text_color(38, 52, 77)
    pdf.cell(0, 5, "Sub-muscles")
    pdf.ln(6)
    ...
    def header_cell(w, h, text="", align="C"):
        pdf.cell(w, h, text, border=1, align=align, fill=True)

    pdf.set_fill_color(38, 52, 77)
    pdf.set_draw_color(55, 71, 98)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("Helvetica", "B", 6)

    x0 = pdf.l_margin
    y0 = pdf.get_y()
    header_cell(col_w[0], 12, "MUSCLE", align="L")
    pdf.set_xy(x0 + col_w[0], y0)
    header_cell(left_total, 6, "LEFT")
    header_cell(right_total, 6, "RIGHT")
    pdf.set_xy(x0 + col_w[0], y0 + 6)
    header_cell(col_w[1], 6, "")
    header_cell(col_w[2], 6, "VOL")
    header_cell(col_w[3], 6, "%")
    header_cell(col_w[4], 6, "")
    header_cell(col_w[5], 6, "VOL")
    header_cell(col_w[6], 6, "%")
    pdf.set_y(y0 + 12)

    pdf.set_font("Helvetica", "", 6.5)
    for index, row in enumerate(rows or []):
        _report_ensure_space(pdf, 8)
        ...
        values = [
            _report_text(row.get("label")),
            "",
            _report_text(left.get("volume")),
            _report_text(left.get("percent")),
            "",
            _report_text(right.get("volume")),
            _report_text(right.get("percent")),
        ]
        aligns = ["L", "C", "R", "R", "C", "R", "R"]

        for value, w, align in zip(values, col_w, aligns):
            pdf.cell(w, 8, value, border=0 if is_total else 1, align=align, fill=fill)
        ...
        _report_draw_swatch(pdf, x0 + col_w[0] + 2.2, y + 2.8, left.get("color"))
        _report_draw_swatch(
            pdf,
            x0 + col_w[0] + left_total + 2.2,
            y + 2.8,
            right.get("color"),
        )
        pdf.set_y(y + 8)
