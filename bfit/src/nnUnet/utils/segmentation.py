...
# ============================================================
def run_segmentation_command(file_path: str,
                             region: str,
                             modality: str,
                             variant: str,
    ...
    # Ensure NIfTI is gzipped
    nii_gz_path = compress_nii_to_nii_gz(file_path)

    case_id = os.path.basename(nii_gz_path).replace(".nii.gz", "")
...
segmentation_commands = {
    ...
    # -------- THIGH --------
    ("Thigh", "MRI", "4class"):
        "nnUNetv2_predict -i {input_file} -o {output_dir} -d 697 -c 3d_fullres -tr nnUNetTrainerTopK10Loss_33os_1000epochs -p nnUNetPlans -f 0",

    ("Thigh", "MRI", "5class"):
        "nnUNetv2_predict -i {input_file} -o {output_dir} -d 700 -c 3d_fullres -tr nnUNetTrainerTopK10Loss_33os_1000epochs -p nnUNetPlans -f 0",

    ("Thigh", "MRI", "48class"):
        "nnUNetv2_predict -i {input_file} -o {output_dir} -d 701 -c 3d_fullres -tr nnUNetTrainerTopK10Loss_33os_1000epochs -p nnUNetPlans -f 0",
}
